...
module.exports.HakimRun = async function ({ api, event, args }) {
	...
	if (!args[0]) return out("الرجاء إدخال ايدي المستخدم / رابط لإضافته.");
	...
	async function adduser(id, name) {
		id = parseInt(id);
		if (participantIDs.includes(id)) return out(`المستخدم ${name ? name : "عضو"} موجود بالمجموعة.`);
		else {
			...
			try {
				...
				return out(`لم أتمكن من أضافة المستخدم ${name ? name : "عضو"} للمجموعة.`);
			}
			if (approvalMode === true && !admins.includes(botID)) return out(`تم أضافة المستخدم ${name ? name : "عضو"} ألى طلبات الأنضمام !`);
			else return out(`تم أضافة المستخدم ${name ? name : "عضو"} للمجموعة !`)
		}
	}
}